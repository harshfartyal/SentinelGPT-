...
def test_suspicious_login():
    ...
    assert len(alerts) == 1
    assert alerts[0]["type"] == "Suspicious Authentication Activity"
...
def test_privilege_escalation():
    ...
    assert len(alerts) == 1
    assert alerts[0]["type"] == "Potential Privilege Escalation"
...
def test_security_group_change():
    ...
    assert len(alerts) == 1
    assert alerts[0]["type"] == "Security Group Modification"
...
def test_root_account_activity():
    ...
    assert len(alerts) == 1
    assert alerts[0]["type"] == "Root Account Activity"
...
def test_cloudtrail_logging_modification():
    ...
    assert len(alerts) == 1
    assert alerts[0]["type"] == "CloudTrail Logging Modification"
